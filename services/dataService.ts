...
const initialSettings: GlobalSettings = {
  ...
  standardTimeLimit: 5400 // 90 minutes (1hr 30min)
};
...
const generateProblems = (): Problem[] => {
  ...
  const languages = ['Python', 'Java', 'C', 'C++', 'AI'];
  
  // Generating a base set of 10+ problems to fill the exam
  for (let i = 1; i <= 15; i++) {
    const lang = languages[i % languages.length];
    problems.push({
      id: `prob-${i}`,
      title: `${lang}: Logic Unit ${i}`,
      description: `Implement a technical logic unit for ${lang}. Scenario: Solve the specified algorithmic complexity requirement for industrial data processing. Case #${i}.`,
      language: lang,
      module: 'Advanced Assessment',
      difficulty: i < 5 ? 'L0' : i < 10 ? 'L1' : 'L2',
      points: 20 + (i * 5),
      starterCode: lang === 'Java' 
        ? 'public class Main {\n    public static void main(String[] args) {\n        // Logic here\n    }\n}' 
        : lang === 'Python' ? '# Candidate logic start\n\n' : '/* Exam Terminal Ready */\n',
      testCases: [
        { input: '10', expectedOutput: '20', isHidden: false },
        { input: '50', expectedOutput: '100', isHidden: true }
      ]
    });
  }
  return problems;
...
export const dataService = {
  ...
  getLanguages: () => ['Python', 'Java', 'C', 'C++', 'AI'],
