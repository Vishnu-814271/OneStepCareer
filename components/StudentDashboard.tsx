...
const StudentDashboard: React.FC<StudentDashboardProps> = ({ user, onLogout }) => {
  ...
  return (
    <div className="min-h-screen flex bg-slate-50 overflow-hidden">
      {/* Sidebar */}
      <aside className="w-20 md:w-64 bg-[#1e293b] flex flex-col sticky top-0 h-screen z-40 text-white shadow-xl shrink-0">
        ...
        <nav className="flex-1 p-4 space-y-1 mt-4">
          <button 
            ...
            className={`w-full p-3 rounded-xl flex items-center gap-3 transition-all ${activeView === 'curriculum' ? 'bg-white/10 text-white shadow-inner' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
          >
            <BookOpen size={18} />
            <span className="hidden md:block font-bold text-[10px] uppercase tracking-widest">Academy Hub</span>
          </button>
          
          <button 
            onClick={() => { setActiveView('career'); }} 
            className={`w-full p-3 rounded-xl flex items-center gap-3 transition-all ${activeView === 'career' ? 'bg-white/10 text-white shadow-inner' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
          >
            <FolderGit2 size={18} />
            <span className="hidden md:block font-bold text-[10px] uppercase tracking-widest">Career Portals</span>
          </button>
        </nav>

        <div className="p-4 mt-auto">
           <button onClick={onLogout} className="w-full p-3 rounded-xl text-slate-500 hover:text-red-400 hover:bg-red-400/5 transition-all flex items-center gap-3 font-bold uppercase text-[10px] tracking-widest">
             <LogOut size={18} />
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-10 shrink-0">
          <div className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest">
             ...
             {activeView === 'career' && <><ChevronRight size={12}/> <span className="text-[#ff8c00]">Industrial Resume Engine</span></>}
        ...
        <main className={`flex-1 overflow-hidden flex flex-col ${activeView === 'career' ? 'p-0' : 'p-10'}`}>
          {activeView === 'curriculum' && (
            <div className="flex-1 overflow-y-auto custom-scrollbar">
               {/* LEVEL 1: COURSE SELECTION */}
               {!selectedCourse && (
                  <div className="max-w-6xl mx-auto space-y-10 animate-fade-in pb-20">
                    <h1 className="text-4xl font-heading font-black text-slate-900 uppercase tracking-tighter">Available <span className="text-slate-400">Certifications</span></h1>
                  ...
                  <div className="max-w-6xl mx-auto space-y-8 animate-fade-in pb-20">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {dataService.getModulesByCategory(selectedCourse, selectedTopicGroup).map((mod) => (
                            <div key={mod.id} onClick={() => handleModuleSelect(mod)} className="bg-white p-8 rounded-3xl border border-slate-200 cursor-pointer group flex items-center gap-6 hover:border-[#ff8c00] shadow-sm transition-all duration-500">
                               ...
                               <div className="flex-1">
                                  <h3 className="text-lg font-black text-slate-900 uppercase tracking-tight leading-none mb-1">{mod.title}</h3>
                                  <p className="text-slate-400 text-[9px] uppercase font-black tracking-[0.2em]">Technical Practice Environment</p>
                               </div>
                               <ChevronRight size={24} className="text-slate-100 group-hover:text-brand-orange transition-colors" />
                            </div>
                        ))}
                      </div>
                  </div>
               ...
               {selectedCourse && selectedTopicGroup && selectedModule && (
                  <div className="max-w-4xl mx-auto space-y-8 animate-fade-in pb-20">
                      ...
                      <div className="bg-white p-12 rounded-[40px] border border-slate-200 shadow-2xl shadow-slate-200/50 space-y-12">
                        <div>
                            <h1 className="text-4xl font-heading font-black text-slate-900 uppercase tracking-tighter">{selectedModule.title}</h1>
                        ...
                        <div className="grid gap-4">
                            {[
                               ...
                               <div 
                                 ...
                                 className={`p-8 rounded-3xl border-2 transition-all duration-300 cursor-pointer flex items-center justify-between ${selectedLevel === level.id ? 'bg-slate-900 border-slate-900 text-white shadow-2xl scale-[1.02]' : 'bg-white border-slate-100 hover:border-slate-300 text-slate-600'}`}
                               >
                                  <div className="flex items-center gap-6 font-black uppercase text-xs tracking-[0.2em]">
                                     <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${selectedLevel === level.id ? 'bg-white/10 text-cyan-400' : 'bg-slate-50 text-slate-300'}`}><Layers size={18}/></div>
                                     {level.title}
                                  </div>
