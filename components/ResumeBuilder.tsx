...
import { FileText, Briefcase, Sparkles, Download, PenTool, Layout, Printer, Loader2, Palette, ChevronDown, ChevronUp, Plus, Trash2, Save, User as UserIcon, GraduationCap, Code2, FolderGit2, Award } from 'lucide-react';
...
const ResumeBuilder: React.FC<ResumeBuilderProps> = ({ currentUser }) => {
  ...
  const AccordionHeader = ({ title, section, icon: Icon }: { title: string, section: SectionType, icon: any }) => (
    <button 
      ...
      className={`w-full flex items-center justify-between p-4 rounded-xl transition-all border border-transparent ${activeSection === section ? 'bg-brand-blue text-white shadow-lg' : 'bg-white text-slate-700 hover:bg-slate-50 border-slate-200'}`}
    >
  ...
  // Common Input Class
  const inputClass = "w-full p-3 rounded-lg border border-slate-300 bg-slate-50 text-slate-900 text-xs font-medium placeholder:text-slate-400 focus:border-brand-cyan focus:ring-1 focus:ring-brand-cyan/20 outline-none transition-all";
  const textAreaClass = "w-full p-3 rounded-lg border border-slate-300 bg-slate-50 text-slate-900 text-xs font-medium placeholder:text-slate-400 focus:border-brand-cyan focus:ring-1 focus:ring-brand-cyan/20 outline-none resize-none transition-all";
    ...
    <div className="w-[210mm] min-h-[297mm] bg-white text-slate-800 flex shadow-2xl mx-auto overflow-hidden text-sm leading-relaxed">
      ...
      <div className="flex-1 p-10 bg-white pt-12">
         ...
         {data.achievements && data.achievements.length > 0 && (
             <div className="mb-10">
                <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-3">Key Achievements</h3>
                <ul className="space-y-2">
                    {data.achievements.map((ach, i) => (
                        <li key={i} className="text-sm text-slate-600 leading-6 pl-4 relative before:content-['★'] before:absolute before:left-0 before:text-brand-cyan before:text-[10px] before:top-1">{ach}</li>
                    ))}
                </ul>
             </div>
         )}
  ...
  const ClassicTemplate = ({ data }: { data: ResumeData }) => (
    <div className="w-[210mm] min-h-[297mm] bg-white text-slate-900 px-20 py-16 mx-auto shadow-2xl serif">
       ...
       {data.achievements && data.achievements.length > 0 && (
           <section className="mb-8">
              <h3 className="font-bold text-lg uppercase border-b border-slate-300 mb-4 pb-1">Achievements & Awards</h3>
              <ul className="list-disc pl-5 space-y-1.5">
                  {data.achievements.map((ach, i) => (
                      <li key={i} className="text-sm text-slate-700 leading-6">{ach}</li>
                  ))}
              </ul>
           </section>
       )}
    ...
    <div className="w-[210mm] min-h-[297mm] bg-white text-slate-800 p-16 mx-auto shadow-2xl font-sans">
       ...
       <div className="grid grid-cols-12 gap-12">
          ...
          <div className="col-span-8 space-y-12">
             ...
             {data.achievements && data.achievements.length > 0 && (
                 <div>
                    <h3 className="text-xs font-bold text-slate-900 uppercase tracking-widest mb-6 border-t-2 border-slate-900 pt-2">Achievements</h3>
                    <ul className="space-y-2">
                        {data.achievements.map((ach, i) => (
                            <li key={i} className="text-sm text-slate-600 leading-7 relative pl-4 before:content-['+'] before:absolute before:left-0 before:text-slate-300">{ach}</li>
                        ))}
                    </ul>
                 </div>
             )}
  ...
  return (
    <div className="max-w-7xl mx-auto min-h-screen pb-20 animate-fade-in flex flex-col lg:flex-row gap-10 px-6 lg:px-12 pt-6">
      {/* LEFT: Input Form (Scrolls with page) */}
      <div className="w-full lg:w-5/12 flex flex-col gap-8">
         ...
         {/* AI Card */}
         <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-xl shadow-brand-blue/5 relative overflow-hidden group">
             <div className="absolute top-0 right-0 w-32 h-32 bg-brand-cyan/5 rounded-full -translate-y-1/2 translate-x-1/2 group-hover:bg-brand-cyan/10 transition-colors"></div>
             ...
             <div className="relative z-10 space-y-4">
                 ...
                 <textarea 
                   ...
                   placeholder="Paste JD here for AI keyword optimization..."
                 ...
                 <button 
                    ...
                    className="w-full py-4 bg-gradient-to-r from-brand-blue to-brand-cyan hover:from-brand-cyan hover:to-brand-blue text-white rounded-xl font-black text-xs uppercase tracking-[0.2em] shadow-lg flex items-center justify-center gap-3 transition-all transform hover:-translate-y-1 disabled:opacity-70 disabled:hover:translate-y-0"
                 >
                    {isGenerating ? <Loader2 size={16} className="animate-spin" /> : <Sparkles size={16} />}
                    {isGenerating ? 'ANALYZING PROFILE...' : 'OPTIMIZE RESUME'}
                 </button>
             </div>
         </div>
         ...
         <div className="space-y-4">
             ...
             <div className={`bg-white rounded-2xl border transition-all duration-300 ${activeSection === 'PERSONAL' ? 'border-brand-cyan shadow-lg shadow-brand-cyan/10 ring-1 ring-brand-cyan/50' : 'border-slate-200 hover:border-slate-300'}`}>
                <AccordionHeader title="Personal Information" section="PERSONAL" icon={UserIcon} />
                {activeSection === 'PERSONAL' && (
                  <div className="p-6 pt-0 space-y-4 animate-in slide-in-from-top-2">
                     <div className="grid grid-cols-1 gap-4">
                        <input type="text" placeholder="Full Name" value={resumeData.personalInfo.fullName} onChange={(e) => handlePersonalChange('fullName', e.target.value)} className={inputClass} />
                        <input type="text" placeholder="Role Title (e.g. Software Engineer)" value={resumeData.personalInfo.role} onChange={(e) => handlePersonalChange('role', e.target.value)} className={inputClass} />
                        <div className="grid grid-cols-2 gap-3">
                           <input type="text" placeholder="Email" value={resumeData.personalInfo.email} onChange={(e) => handlePersonalChange('email', e.target.value)} className={inputClass} />
                           <input type="text" placeholder="Phone" value={resumeData.personalInfo.phone} onChange={(e) => handlePersonalChange('phone', e.target.value)} className={inputClass} />
                        </div>
                        <input type="text" placeholder="Location" value={resumeData.personalInfo.location} onChange={(e) => handlePersonalChange('location', e.target.value)} className={inputClass} />
                        <input type="text" placeholder="LinkedIn URL" value={resumeData.personalInfo.linkedin} onChange={(e) => handlePersonalChange('linkedin', e.target.value)} className={inputClass} />
                     </div>
                  </div>
                )}
             </div>
             ...
             {/* Summary */}
             <div className={`bg-white rounded-2xl border transition-all duration-300 ${activeSection === 'SUMMARY' ? 'border-brand-cyan shadow-lg shadow-brand-cyan/10 ring-1 ring-brand-cyan/50' : 'border-slate-200 hover:border-slate-300'}`}>
             ...
             {/* Achievements (NEW) */}
             <div className={`bg-white rounded-2xl border transition-all duration-300 ${activeSection === 'ACHIEVEMENTS' ? 'border-brand-cyan shadow-lg shadow-brand-cyan/10 ring-1 ring-brand-cyan/50' : 'border-slate-200 hover:border-slate-300'}`}>
                <AccordionHeader title="Achievements & Awards" section="ACHIEVEMENTS" icon={Award} />
                {activeSection === 'ACHIEVEMENTS' && (
                  <div className="p-6 pt-0 animate-in slide-in-from-top-2">
                     ...
                     />
                     <p className="text-[10px] text-slate-400 mt-2 text-right">Separate each achievement with a new line.</p>
                  </div>
                )}
             </div>

             {/* Experience */}
             <div className={`bg-white rounded-2xl border transition-all duration-300 ${activeSection === 'EXPERIENCE' ? 'border-brand-cyan shadow-lg shadow-brand-cyan/10 ring-1 ring-brand-cyan/50' : 'border-slate-200 hover:border-slate-300'}`}>
                <AccordionHeader title="Experience" section="EXPERIENCE" icon={Briefcase} />
                {activeSection === 'EXPERIENCE' && (
                   <div className="p-6 pt-0 space-y-4 animate-in slide-in-from-top-2">
                      {resumeData.experience.map((exp, idx) => (
                         <div key={exp.id} className="p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-3 relative group">
                            ...
                            <input type="text" placeholder="Job Title" value={exp.role} onChange={(e) => handleArrayChange('experience', exp.id, 'role', e.target.value)} className={inputClass} />
                            <div className="grid grid-cols-2 gap-2">
                               <input type="text" placeholder="Company" value={exp.company} onChange={(e) => handleArrayChange('experience', exp.id, 'company', e.target.value)} className={inputClass} />
                               <input type="text" placeholder="Date Range" value={exp.date} onChange={(e) => handleArrayChange('experience', exp.id, 'date', e.target.value)} className={inputClass} />
                            </div>
                            <textarea 
             ...
             {/* Education */}
             <div className={`bg-white rounded-2xl border transition-all duration-300 ${activeSection === 'EDUCATION' ? 'border-brand-cyan shadow-lg shadow-brand-cyan/10 ring-1 ring-brand-cyan/50' : 'border-slate-200 hover:border-slate-300'}`}>
                <AccordionHeader title="Education" section="EDUCATION" icon={GraduationCap} />
                {activeSection === 'EDUCATION' && (
                   <div className="p-6 pt-0 space-y-4 animate-in slide-in-from-top-2">
                      {resumeData.education.map((edu) => (
                         <div key={edu.id} className="p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-3 relative group">
                            ...
                            <input type="text" placeholder="Degree" value={edu.degree} onChange={(e) => handleArrayChange('education', edu.id, 'degree', e.target.value)} className={inputClass} />
                            <div className="grid grid-cols-2 gap-2">
                               <input type="text" placeholder="School/University" value={edu.school} onChange={(e) => handleArrayChange('education', edu.id, 'school', e.target.value)} className={inputClass} />
                               <input type="text" placeholder="Year" value={edu.date} onChange={(e) => handleArrayChange('education', edu.id, 'date', e.target.value)} className={inputClass} />
                            </div>
                         </div>
                      ))}
             ...
             {/* Skills */}
             <div className={`bg-white rounded-2xl border transition-all duration-300 ${activeSection === 'SKILLS' ? 'border-brand-cyan shadow-lg shadow-brand-cyan/10 ring-1 ring-brand-cyan/50' : 'border-slate-200 hover:border-slate-300'}`}>
                <AccordionHeader title="Skills" section="SKILLS" icon={Code2} />
                {activeSection === 'SKILLS' && (
                  <div className="p-6 pt-0 bg-slate-50 border-t border-slate-100 space-y-4 animate-in slide-in-from-top-2">
                     <div>
                        <label className="text-[10px] font-bold text-slate-400 uppercase block mb-1">Technical (Comma separated)</label>
                        <textarea value={resumeData.skills.technical.join(', ')} onChange={(e) => handleSkillsChange('technical', e.target.value)} className={textAreaClass} style={{ height: '4rem' }} />
                     </div>
                     <div>
                        <label className="text-[10px] font-bold text-slate-400 uppercase block mb-1">Tools</label>
                        <input type="text" value={resumeData.skills.tools.join(', ')} onChange={(e) => handleSkillsChange('tools', e.target.value)} className={inputClass} />
                     </div>
                  </div>
                )}
             </div>
             ...
             {/* Projects */}
             <div className={`bg-white rounded-2xl border transition-all duration-300 ${activeSection === 'PROJECTS' ? 'border-brand-cyan shadow-lg shadow-brand-cyan/10 ring-1 ring-brand-cyan/50' : 'border-slate-200 hover:border-slate-300'}`}>
                <AccordionHeader title="Projects" section="PROJECTS" icon={FolderGit2} />
                {activeSection === 'PROJECTS' && (
                   <div className="p-6 pt-0 space-y-4 animate-in slide-in-from-top-2">
                      {resumeData.projects.map((proj) => (
                         <div key={proj.id} className="p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-3 relative group">
                            ...
                            <input type="text" placeholder="Project Name" value={proj.name} onChange={(e) => handleArrayChange('projects', proj.id, 'name', e.target.value)} className={inputClass} />
                            <input type="text" placeholder="Tech Stack" value={proj.techStack} onChange={(e) => handleArrayChange('projects', proj.id, 'techStack', e.target.value)} className={inputClass} />
                            <textarea placeholder="Description" value={proj.description} onChange={(e) => handleArrayChange('projects', proj.id, 'description', e.target.value)} className={textAreaClass} style={{ height: '4rem' }} />
                         </div>
                      ))}
      ...
      <div className="w-full lg:w-7/12 relative">
          <div className="sticky top-8 space-y-6">
              {/* Toolbar */}
              <div className="bg-white rounded-2xl p-4 border border-slate-200 shadow-lg flex flex-wrap items-center justify-between gap-4 z-20">
                  <div className="flex items-center gap-4">
                     <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                        <Palette size={14}/> Style
                     </span>
                     <div className="flex bg-slate-50 p-1 rounded-lg border border-slate-100">
                        <button onClick={() => setSelectedTemplate('MODERN')} className={`px-4 py-1.5 rounded-md text-[10px] font-bold uppercase tracking-wide transition-all ${selectedTemplate === 'MODERN' ? 'bg-white shadow-sm text-brand-blue ring-1 ring-black/5' : 'text-slate-400 hover:text-slate-600'}`}>Modern</button>
                        <button onClick={() => setSelectedTemplate('CLASSIC')} className={`px-4 py-1.5 rounded-md text-[10px] font-bold uppercase tracking-wide transition-all ${selectedTemplate === 'CLASSIC' ? 'bg-white shadow-sm text-brand-blue ring-1 ring-black/5' : 'text-slate-400 hover:text-slate-600'}`}>Classic</button>
                        <button onClick={() => setSelectedTemplate('MINIMAL')} className={`px-4 py-1.5 rounded-md text-[10px] font-bold uppercase tracking-wide transition-all ${selectedTemplate === 'MINIMAL' ? 'bg-white shadow-sm text-brand-blue ring-1 ring-black/5' : 'text-slate-400 hover:text-slate-600'}`}>Minimal</button>
                     </div>
                  </div>

                  <button 
                     onClick={handlePrint}
                     className="flex items-center gap-2 px-6 py-2 bg-[#0f172a] hover:bg-brand-cyan text-white rounded-xl font-bold text-xs uppercase tracking-wider shadow-lg transition-all hover:scale-105"
                  >
              ...
              {/* Preview Container */}
              <div className="bg-slate-200/50 rounded-3xl p-8 lg:p-12 border border-slate-200/60 shadow-inner flex justify-center min-h-[600px] lg:h-[calc(100vh-140px)] overflow-y-auto custom-scrollbar">
                  <div ref={printRef} className="origin-top transition-transform duration-300 scale-[0.5] sm:scale-[0.6] md:scale-[0.7] lg:scale-[0.85] shadow-2xl bg-white h-fit">
